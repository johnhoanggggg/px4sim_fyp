#!/usr/bin/env python3
"""
DWA3D — Bidirectional 3-D Dynamic Window Approach on a spherical scan.

Instead of finding discrete gaps via flood-fill (like FGM3D), DWA3D
scores every admissible direction on the spherical grid with a weighted
cost function and picks the lowest-cost direction.  Being bidirectional,
backward directions are valid candidates (penalised but available),
which helps the drone back out of tight spaces.

Uses the same spherical grid infrastructure as FGM3D:
    - n_az   azimuth bins   covering [-pi, pi)      (horizontal)
    - n_el   elevation bins covering [-el_max, el_max] (vertical)
    - Sensor coverage mask, bubble inflation, range map
...
class DWA3D:
    """
    Bidirectional 3-D Dynamic Window Approach on a spherical scan.
    ...
    safety_margin_cells : int
        Number of neighbour cells that must also be free around a candidate.
    clearance_radius_cells : int
        Radius (in cells) for obstacle clearance lookup in range_map.
    el_max_deg : float
    ...
    def update(self, obstacle_pts: np.ndarray, goal_body: tuple) -> tuple:
        ...
        # 3. Compute admissible directions (dilate blocked mask for safety)
        admissible = self._compute_admissible()
        ...
        # 4. Score all admissible candidates
        total_cost = self._score_candidates(admissible, goal_dir)
    ...
    # ------------------------------------------------------------------
    # Spherical map building (same as FGM3D)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # DWA-specific: admissibility and cost scoring
    # ------------------------------------------------------------------

    def _compute_admissible(self) -> np.ndarray:
    ...
    def _min_range_nearby(self) -> np.ndarray:
        ...
        Pure numpy rolling-min (no scipy).
        """
    ...
    def _score_candidates(self, admissible: np.ndarray,
                          goal_dir: np.ndarray) -> np.ndarray:
        """Score all cells. Inadmissible cells get inf cost."""
        ...
        # --- cost_obstacle: inverse clearance ---
...
if __name__ == "__main__":
    ...
    vel = dwa.update(beam, (2.0, 0.0, 0.0))
    ...
    vel = dwa.update(beam_above, (2.0, 0.0, 0.3))
    ...
    vel = dwa.update(rafter, (2.0, 0.0, -0.5))
    ...
    dwa.reset()
    vel = dwa.update(np.empty((0, 3)), (2.0, 0.5, 1.0))
    print(f"\nNo obstacles → vel=({vel[0]:.3f}, {vel[1]:.3f}, {vel[2]:.3f})")
    print(f"  Should point toward goal")

    # U-trap — should back out
    dwa.reset()
    trap = np.array([
        # Wall ahead
        [0.8, y, 0.0] for y in np.linspace(-0.5, 0.5, 8)
    ] + [
        # Wall left
        [x, 0.6, 0.0] for x in np.linspace(-0.2, 0.8, 8)
    ] + [
        # Wall right
        [x, -0.6, 0.0] for x in np.linspace(-0.2, 0.8, 8)
    ])
    vel = dwa.update(trap, (2.0, 0.0, 0.0))
    print(f"\nU-trap, goal forward → vel=({vel[0]:.3f}, {vel[1]:.3f}, {vel[2]:.3f})")
    print(f"  Should back out (vx negative) since forward is blocked")
