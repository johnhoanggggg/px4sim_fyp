#!/usr/bin/env python3
"""
Offboard control with VFH*2D obstacle avoidance through a pillar field.

Same mission as fly_pillars.py but uses VFH*2D (multi-step lookahead)
instead of plain VFH2D.  The lookahead tree lets the planner avoid
dead-ends and U-shaped traps that greedy VFH walks into.

Usage:
    1. Start PX4 SITL:
         cd ~/PX4-Autopilot && PX4_GZ_WORLD=pillars make px4_sitl gz_x500_tof
    2. Run this script:
         python3 fly_pillars_star.py
"""
...
from vfh2d_star import VFH2DStar
...
vfh = VFH2DStar(
    ...
    drone_radius=0.5,
    enlarge_bins=2,
    enlarge_close_bins=4,
    enlarge_close_threshold=0.5,
...
def fly_with_avoidance(waypoints, wp_offset=0):
    """Navigate waypoints using VFH*2D obstacle avoidance."""
    global use_pos_setpoints
    use_pos_setpoints = False

    for i, (wx, wy, wz, label) in enumerate(waypoints):
        wp_idx = i + wp_offset
        print(f"\n>>> [{wp_idx+1}/{len(WAYPOINTS)}] {label}  (VFH* depth={LOOKAHEAD_DEPTH})")
        t0 = time.time()
...
try:
    ...
    # Fly pillar field with VFH*2D
    print(f"\nEnabling VFH*2D obstacle avoidance (depth={LOOKAHEAD_DEPTH}, step={STEP_DISTANCE}m)...")
    vfh.reset()
