#!/usr/bin/env python3
"""
Offboard control with APF (Artificial Potential Field) obstacle avoidance
through a pillar field.

Replaces VFH*2D with a continuous potential field approach that avoids the
pillar-clipping problem caused by angular bin discretization.

Usage:
    1. Start PX4 SITL:
         cd ~/PX4-Autopilot && PX4_GZ_WORLD=pillars make px4_sitl gz_x500_tof
    2. Run this script:
         python3 fly_pillars_apf.py
"""
...
def fly_with_avoidance(waypoints, wp_offset=0):
    ...
    for i, (wx, wy, wz, label) in enumerate(waypoints):
        ...
        while time.time() - t0 < 60:
            ...
            # Obstacles
            pts = tof.get_obstacle_points(max_range=2.5)
            if len(pts) > 0:
                pts = pts[(pts[:, 2] > -0.5) & (pts[:, 2] < 1.0)]

            # APF
            if len(pts) > 0:
                vel_body = apf.update(pts, goal_body)
            else:
...
try:
    ...
    # Fly pillar field with APF
    print(f"\nEnabling APF obstacle avoidance...")
    apf.reset()
