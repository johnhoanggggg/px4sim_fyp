...
class VFH3D:
    ...
    def update(self, obstacle_pts: np.ndarray, goal_body: tuple) -> tuple:
        ...
        Returns:
            (vx, vy, vz, yaw_target) velocity and target yaw in body FLU.
        """
    ...
    def _retreat(self, pts: np.ndarray, min_obs_dist: float) -> tuple:
        ...
        if norm < 0.01:
            return (-speed, 0.0, 0.0, math.pi)
        
        retreat_dir = -centroid / norm
        retreat_az = math.atan2(retreat_dir[1], retreat_dir[0])
...
if __name__ == "__main__":
    ...
    print(f"Beam ahead → vel=({vel[0]:.3f}, {vel[1]:.3f}, {vel[2]:.3f}, yaw={math.degrees(vel[3]):.1f}°)")
    ...
    print(f"\nBeam above, goal up → vel=({vel[0]:.3f}, {vel[1]:.3f}, {vel[2]:.3f}, yaw={math.degrees(vel[3]):.1f}°)")
    ...
    print(f"\nRafter blocking fwd-up → vel=({vel[0]:.3f}, {vel[1]:.3f}, {vel[2]:.3f}, yaw={math.degrees(vel[3]):.1f}°)")
    print(f"  Should go forward-and-down (vz negative)")

    # No obstacles — straight to goal
    vfh.reset()
    vel = vfh.update(np.empty((0, 3)), (2.0, 0.5, 1.0))
    print(f"\nNo obstacles → vel=({vel[0]:.3f}, {vel[1]:.3f}, {vel[2]:.3f}, yaw={math.degrees(vel[3]):.1f}°)")
